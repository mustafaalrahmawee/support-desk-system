<template>
  <div class="flex min-h-screen bg-[#0c0e14] font-[Sora,sans-serif]">
    ...
    <!-- Left: Brand panel -->
    <aside class="relative hidden lg:flex w-[380px] shrink-0 flex-col justify-between border-r border-[#1e2130] bg-[#0f1117] px-10 py-12 overflow-hidden">
      <!-- Grid texture -->
      <div
        class="pointer-events-none absolute inset-0"
        style="background-image: linear-gradient(rgba(245,158,11,.04) 1px, transparent 1px), linear-gradient(90deg, rgba(245,158,11,.04) 1px, transparent 1px); background-size: 32px 32px;"
        aria-hidden="true"
      ></div>
      ...
      <div class="relative z-10">
        ...
        <svg class="mb-8" width="40" height="40" viewBox="0 0 40 40" fill="none">
          ...
          <rect x="22" y="2" width="16" height="16" stroke="#f59e0b" stroke-width="2" opacity="0.5"/>
          <rect x="2" y="22" width="16" height="16" stroke="#f59e0b" stroke-width="2" opacity="0.5"/>
        ...
        <ul class="space-y-3">
          <li v-for="feat in features" :key="feat" class="flex items-center gap-2.5 text-sm font-light text-slate-400">
      ...
      <p class="relative z-10 font-mono text-[.65rem] tracking-widest text-slate-700">v1.0 · Internes System</p>
    </aside>
    ...
    <main class="flex flex-1 items-center justify-center px-6 py-10">
      <div class="w-full max-w-[420px]">
        ...
        <!-- Header -->
        <p class="mb-3 font-mono text-[.65rem] font-semibold uppercase tracking-[.2em] text-amber-500">ANMELDUNG</p>
        <h2 class="mb-2 text-3xl font-bold tracking-tight text-slate-100">Willkommen zurück</h2>
        <p class="mb-8 text-sm font-light text-slate-500">Melden Sie sich mit Ihren Zugangsdaten an.</p>
        ...
        <form class="space-y-5" @submit.prevent="handleSubmit" novalidate>
          ...
          <!-- Email -->
          <div>
            <label class="mb-1.5 block text-[.72rem] font-semibold uppercase tracking-[.08em] text-slate-400" for="email">
              E-Mail-Adresse
            </label>
            <input
              id="email"
            ...
            <p v-if="v$.email.$error" class="mt-1.5 font-mono text-xs text-red-400">
              {{ v$.email.required.$invalid ? 'E-Mail-Adresse ist erforderlich.' : 'Bitte eine gültige E-Mail-Adresse eingeben.' }}
            </p>
          </div>

          <!-- Password -->
          <div>
            <label class="mb-1.5 block text-[.72rem] font-semibold uppercase tracking-[.08em] text-slate-400" for="password">
              Passwort
            </label>
            <div class="relative">
              <input
            ...
            <p v-if="v$.password.$error" class="mt-1.5 font-mono text-xs text-red-400">Passwort ist erforderlich.</p>
          </div>
          ...
          >
            <svg v-if="uiState === 'loading'" class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none">
